    ...
    public class ParcelType:DataBaseStorageHelper
    {
        ...
        public async void AddTact(int alltacts)
        {
            ...
            await Task.Run(new Action(() => {
                ...
                try
                {
                    if (this.CurrentTactNumber == 300)                    {
                        
                        TimeSpan ts = DateTime.Now - this.CreateTime;
                        this.processingtime = (int)ts.TotalMilliseconds;
                        MysqlCore.DB_Main().ExecuteNonQuery("UPDATE parcels set processingtime='" + (int)ts.TotalMilliseconds + "' where id=" + this.Id);
                    }
                }
                catch (Exception)
                {
         ...
         if (this.DestinationStand.Name == "Odrzuty")
                {
                    ...
                    this.DestinationStandItem.Lamp1OnTacts = 6;
                }
                else
                {
                    this.DestinationStandItem.Lamp1OnTacts = 6;
